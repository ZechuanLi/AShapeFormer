...
def compute_vote_loss(end_points):
    ...
    # print("vote_xyz_reshape.shape",vote_xyz_reshape.shape)
    # print("seed_gt_votes_reshape.shape",seed_gt_votes_reshape.shape)
################## dir_loss
    # gt_offsets_norm = torch.norm(seed_gt_votes_reshape, p=2, dim=1)   # (N), float
    # gt_offsets_ = seed_gt_votes_reshape / (gt_offsets_norm.unsqueeze(-1) + 1e-8)
    # pt_offsets_norm = torch.norm(vote_xyz_reshape, p=2, dim=1)
    # pt_offsets_ = vote_xyz_reshape / (pt_offsets_norm.unsqueeze(-1) + 1e-8)
    # direction_diff = - (gt_offsets_ * pt_offsets_).sum(-1)   # (N)
    # print(direction_diff.shape)
    # direction_diff = direction_diff.view(batch_size, num_seed)
    # offset_dir_loss = torch.sum(direction_diff * seed_gt_votes_mask.float()) / (torch.sum(*seed_gt_votes_mask.float()) + 1e-6)
################## dir_loss
    ...
    # vote_loss = vote_loss + offset_dir_loss
    return vote_loss
...
def compute_sem_cls_loss(end_points):
    """ Compute seed point semantic classification loss.
    ...
    semantic_preds = end_points['semantic_preds']
    seed_gt_votes_mask = torch.gather(end_points['vote_label_mask'], 1, seed_inds)
    
    sem_cls_label = seed_gt_votes_mask
  
    criterion_sem_cls = nn.CrossEntropyLoss(reduction='none')
    # score_pred_sigmoid = end_points['score_pred_sigmoid']
    # correct = (score_pred_sigmoid.ge(0.5)==sem_cls_label).sun().item()
    # n = sem_cls_label.shape[0]*sem_cls_label.shape[1]
    # acc = correct/n
    # print("sem cls acc is :",acc)
    #sem_cls_loss = criterion_sem_cls(semantic_preds.transpose(2,1), sem_cls_label) # (B,K)
    sem_cls_loss = criterion_sem_cls(semantic_preds, sem_cls_label) # (B,K)
    sem_cls_loss = torch.sum(sem_cls_loss * sem_cls_label)/(torch.sum(sem_cls_label)+1e-6)
    #print("sem_cls_loss",sem_cls_loss)
    return sem_cls_loss
...
def get_loss(end_points, config):
    ...
    obj_cls_loss_hard_topk = compute_points_obj_cls_loss_hard_topk(end_points,4)
    end_points['obj_cls_loss_hard_topk'] = obj_cls_loss_hard_topk
    # Final loss function
    loss = obj_cls_loss_hard_topk + vote_loss + 0.5*objectness_loss + box_loss + 0.1*sem_cls_loss 
