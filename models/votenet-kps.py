...
class VoteNet(nn.Module):
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
        ...
        # seg 
        self.points_obj_cls = PointsObjClsModule(256)
        self.seg1 = torch.nn.Conv1d(256, 128, 1)
        self.seg2 = torch.nn.Conv1d(128, 64, 1)
        self.seg3 = torch.nn.Conv1d(64, 2, 1)
    ...
    def forward(self, inputs):
        ...
        #scores_topk = torch.topk(points_obj_cls_scores, self.num_proposal)[0]      
        
        xyz, features = self.vgen(xyz, features) # vote_xyz vote_feature
        relu = nn.ReLU()
        weight = points_obj_cls_scores.unsqueeze(-1)
        weight = weight.repeat(1,1,256)     
        weight = weight.transpose(1,2)
        
        seed_features_w = end_points['seed_features'] * weight
        vote_features_w = features * weight

        features_norm = torch.norm(features, p=2, dim=1)
        features = features.div(features_norm.unsqueeze(1))

        vote_features_w_norm  = torch.norm(vote_features_w, p=2, dim=1)
        vote_features_w = vote_features_w.div(vote_features_w_norm.unsqueeze(1))

        seed_features_w_norm  = torch.norm(seed_features_w, p=2, dim=1)
        seed_features_w = seed_features_w.div(seed_features_w_norm.unsqueeze(1))

        
        end_points['seed_features_w']= seed_features_w
        end_points['vote_features_w']= vote_features_w
