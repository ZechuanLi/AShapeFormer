...
def compute_sem_cls_loss(end_points):
    ...
    Returns:
        sem_cls_loss
    """
    B = end_points['seed_xyz'].shape[0]
    K = end_points['seed_xyz'].shape[1]
    seed_gt_votes_mask = torch.gather(end_points['vote_label_mask'], 1, end_points['seed_inds'].long())

    sem_cls_label = seed_gt_votes_mask
    criterion = nn.BCEWithLogitsLoss()

    cls_loss_src = criterion(end_points['semantic_preds'], sem_cls_label.unsqueeze(1).float())
    sem_cls_objectness_loss = cls_loss_src 

    return sem_cls_objectness_loss
...
def get_loss(end_points, config):
    ...
    # seed sem_cls loss
    seed_sem_cls_loss = compute_sem_cls_loss(end_points) 
    end_points['seed_sem_cls_loss'] = seed_sem_cls_loss
    ...
    # Final loss function
    loss = vote_loss + 0.5*objectness_loss + box_loss + 0.1*sem_cls_loss + seed_sem_cls_loss
